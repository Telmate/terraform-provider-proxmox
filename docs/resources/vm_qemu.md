...
The `disk` block is used to configure the disk devices. It may be specified multiple times. This block does not diff as pretty as the disks block, but it is more flexible for modules.
...
|`type`                |`string`|`disk` |The type of disk to create. Options: `cdrom`, `disk`.|
